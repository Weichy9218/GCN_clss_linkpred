...
if __name__ == '__main__':
    ...
    features, labels, Dense_matrix = citeseer_dataset.load_data()  # 加载数据
    ...
    # 模型、优化器和损失函数
    input_size = features.shape[1]  # 输入特征的维度，例如1433
    ...
    features = F.normalize(features, p=2, dim=-1)  # 特征行归一化

    tensor_adjacency = generate_tensor_adjacency_for_link(edge_of_pos_train, drop_edge=0, nums_node=len(labels)).to(
        device)  # 链接预测任务的稀疏张量表示的邻接矩阵。
    ...
    plt.savefig('./fig/citeseer_link_pairnorm.png')  # 保存图像
    plt.show()
