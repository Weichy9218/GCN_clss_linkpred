...
def train_test_split_edges(
    ...
    # Negative edges.手动修改
    batch_size = 256  # 调整批次大小以适应你的内存
    neg_adj_mask = torch.ones(num_nodes, num_nodes, dtype=torch.uint8)
    neg_adj_mask = neg_adj_mask.triu(diagonal=1).to(torch.bool)
    neg_adj_mask[row, col] = 0
    num_samples = neg_adj_mask.size(0)

    for i in range(0, num_samples, batch_size):
        batch_neg_adj_mask = neg_adj_mask[i:i + batch_size]

        batch_neg_row, batch_neg_col = batch_neg_adj_mask.nonzero(as_tuple=False).t()
        perm = torch.randperm(batch_neg_row.size(0))[:n_v + n_t]
        batch_neg_row, batch_neg_col = batch_neg_row[perm], batch_neg_col[perm]

        batch_neg_adj_mask[batch_neg_row, batch_neg_col] = 0

        if i == 0:
            data.train_neg_adj_mask = batch_neg_adj_mask
        elif i == batch_size:
            row, col = batch_neg_row[:n_v], batch_neg_col[:n_v]
            data.val_neg_edge_index = torch.stack([row, col], dim=0)
        else:
            row, col = batch_neg_row[:n_t], batch_neg_col[:n_t]
