...
sys.path.append("..")
sys.path.append(".")
from PPI.My_train_test_split_edges import train_test_split_edges
...
def generate_tensor_adjacency_for_link(edge_index, drop_edge=0, nums_node=2708, self_link=True):
    ...
    Tensor_adjacency = torch.sparse_coo_tensor(
        ...
        torch.Size([nums_node, nums_node]), dtype=torch.float32)
...
class LinkPredictionGCNModel(nn.Module):
    ...
    def forward(self, x, edge_index, pos_edge_index, neg_edge_index):
        ...
        out_feature = self.layer3(hidden, edge_index)
...
def generate_positive_negative_edges(Dense_matrix):
    ...
    - negative_edges: 负样本边
    """
    nonzero_indices = np.column_stack(Dense_matrix.nonzero())
...
if __name__ == '__main__':
    ...
    init_seeds(seed=1234)  # 随机seed
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

    path_of_ppi = r"./PPI/ppi"
    ppi_dataset = PPIDataFromJson(path_of_ppi)

    tensor_x = torch.tensor(ppi_dataset.feature_of_pg, dtype=torch.float)
    edge_index = torch.tensor(ppi_dataset.edge_of_pg, dtype=torch.long)
    ...
    val_pos_edge = data.val_pos_edge_index.to(device)
    test_pos_edge = data.test_pos_edge_index.to(device)

    val_neg_edge = data.val_neg_edge_index.to(device)
    test_neg_edge = data.test_neg_edge_index.to(device)

    hidden_size = 256  # 隐藏层维度，可以根据需要调整
    output_size = 64  # 输出维度，例如类别数量

    model = LinkPredictionGCNModel(feature_dim, hidden_size, output_size).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        train_output = model(tensor_x, train_pos_edge, train_pos_edge, neg_edge_index)

        targets = get_link_labels(train_pos_edge, neg_edge_index)

        # print(train_output.shape, targets.shape)
        # 计算二元交叉熵损失
        ...
        model.eval()
        with torch.no_grad():
            val_output = model(tensor_x, train_pos_edge, val_pos_edge, val_neg_edge)
            targets = get_link_labels(val_pos_edge, val_neg_edge)
    ...
    model.eval()
    with torch.no_grad():
        test_output = model(tensor_x, train_pos_edge, test_pos_edge, test_neg_edge)
        targets = get_link_labels(test_pos_edge, test_neg_edge)
    ...
    plt.savefig('./fig/PPI_link.png')  # 保存图像
